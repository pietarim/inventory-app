export const devices = [
  { name: 'laptop', manufacturer: 'Dell', deviceNumber: 798122 },
  { name: 'laptop', manufacturer: 'Apple', deviceNumber: 123794 },
  { name: 'laptop', manufacturer: 'Lenovo', deviceNumber: 798798 },
  { name: 'mobile', manufacturer: 'Apple', deviceNumber: 123000 },
  { name: 'mobile', manufacturer: 'Samsung', deviceNumber: 798123 },
  { name: 'mobile', manufacturer: 'Google', deviceNumber: 123711 },
  { name: 'tablet', manufacturer: 'Apple', deviceNumber: 798722 },
  { name: 'tablet', manufacturer: 'Samsung', deviceNumber: 123001 },
  { name: 'tablet', manufacturer: 'Google', deviceNumber: 798111 },
]

export const issuances = [
  {
    receiver: 'John Doe',
    start: new Date('2024', '0', '1'),
    end: new Date('2024', '11', '31'),
    department: 'IT',
  },
  {
    receiver: 'Jane Doe',
    start: new Date('2024', '0', '1'),
    end: new Date('2024', '11', '31'),
    department: 'HR',
  },
  {
    receiver: 'Sirpa Miettinen',
    start: new Date('2024', '0', '1'),
    end: new Date('2024', '11', '31'),
    department: 'Finance',
  },
  {
    receiver: 'Teppo Salmi',
    start: new Date('2023', '0', '1'),
    end: new Date('2023', '11', '31'),
    department: 'IT',
  },
  {
    receiver: 'Johanna Virtanen',
    start: new Date('2022', '0', '1'),
    end: new Date('2022', '11', '31'),
    department: 'IT',
  },
  {
    receiver: 'Saku Virtanen',
    start: new Date('2021', '0', '1'),
    end: new Date('2021', '11', '31'),
    department: 'IT',
  },
  {
    receiver: 'Mikko Virtanen',
    start: new Date('2022', '0', '1'),
    end: new Date('2022', '11', '31'),
    department: 'IT',
  },
  {
    receiver: 'Ida Saari',
    start: new Date('2022', '0', '1'),
    end: new Date('2022', '11', '31'),
    department: 'IT',
  },
  {
    receiver: 'Roope Niemi',
    start: new Date('2022', '0', '1'),
    end: new Date('2022', '11', '31'),
    department: 'IT',
  },
  {
    receiver: 'Saara Virtanen',
    start: new Date('2022', '0', '1'),
    end: new Date('2022', '11', '31'),
    department: 'IT',
  },
  {
    receiver: 'Vilho Koskinen',
    start: new Date('2022', '0', '1'),
    end: new Date('2022', '11', '31'),
    department: 'IT',
  },
  {
    receiver: 'Kalle Virtanen',
    start: new Date('2022', '0', '1'),
    end: new Date('2022', '11', '31'),
    department: 'IT',
  },
  {
    receiver: 'Salla Virtanen',
    start: new Date('2021', '0', '1'),
    end: new Date('2021', '11', '31'),
    department: 'IT',
  },
]
